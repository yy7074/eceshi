...
class WeChatPayService:
    ...
    async def create_h5_payment(
        ...
        params = {
            ...
            'scene_info': '{"h5_info": {"type":"Wap","app_name":"E测试","package_name":"eceshi"}}'
        }
        
        # 生成签名
        params['sign'] = self.generate_sign(params)
        
        print(f"[订单H5支付] 统一下单参数: {params}")
        ...
        try:
            ...
            if result.get('return_code') != 'SUCCESS':
                error_msg = result.get('return_msg', '未知错误')
                print(f"[订单H5支付] 统一下单失败: {error_msg}")
                raise Exception(f"微信支付统一下单失败: {error_msg}")
            
            if result.get('result_code') != 'SUCCESS':
                error_code = result.get('err_code', '')
                error_desc = result.get('err_code_des', '未知错误')
                print(f"[订单H5支付] 支付失败: {error_code} - {error_desc}")
                raise Exception(f"微信支付错误: {error_desc}")
            
            # 获取mweb_url
            mweb_url = result.get('mweb_url')
            if not mweb_url:
                raise Exception("未获取到mweb_url")
            
            print(f"[订单H5支付] 获取mweb_url成功: {mweb_url}")
            
        except Exception as e:
            print(f"[订单H5支付] 调用微信API异常: {str(e)}")
            raise Exception(f"创建支付订单失败: {str(e)}")
